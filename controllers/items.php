...
class Items extends CI_Controller {
	...
	public function view($catID = NULL, $page = 1) {
        ...
        foreach ($this->Items_model->getAll($catID) as $item) {
            ...
            $this->table->add_row(img($item['ImageSmall']), anchor('items/viewitem/'.$item['ID'], $item['Name']), $item['Price'], anchor('items/edititem/'.$item['ID'], 'Edit '.$typeName), anchor('items/deleteitem/'.$item['ID'], 'Delete '.$typeName, array('class' => 'modalconfirm', 'data-modaltext' => $prompt)));
        }
